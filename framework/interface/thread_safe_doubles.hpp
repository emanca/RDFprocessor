...
#ifndef BOOST_HISTOGRAM_ACCUMULATORS_THREAD_SAFE_DOUBLES_HPP
#define BOOST_HISTOGRAM_ACCUMULATORS_THREAD_SAFE_DOUBLES_HPP
 ...
 */
            template <class T>
            class thread_safe_doubles : public std::atomic<T>
            {
            public:
                using value_type = T;
                using super_t = std::atomic<T>;

                thread_safe_doubles() noexcept : super_t(static_cast<T>(0)) {}
                // non-atomic copy and assign is allowed, because storage is locked in this case
                thread_safe_doubles(const thread_safe_doubles &o) noexcept : super_t(o.load()) {}
                thread_safe_doubles &operator=(const thread_safe_doubles &o) noexcept
                {
                    super_t::store(o.load());
                    return *this;
                }

                thread_safe_doubles(value_type arg) : super_t(arg) {}
                thread_safe_doubles &operator=(value_type arg)
                {
                    super_t::store(arg);
                    return *this;
                }

                thread_safe_doubles &operator+=(const thread_safe_doubles &arg)
                {
                    operator+=(arg.load());
                    return *this;
                }
